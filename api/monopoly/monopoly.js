...
function getInfo(fs, io) {
    fs.readFile('./database/monopolyroom.txt', 'utf8', (err, jsonFile) => {
        if (err) {
            return
        }
...
function create(fs, io, data) {
    fs.readFile('./database/monopolyroom.txt', 'utf8', (err, jsonFile) => {
        ...
        fs.writeFile('./database/monopolyroom.txt', JSON.stringify(arr), (err) => {
            if (err) throw err;
        });
...
function start(fs, io, data) {
    fs.readFile('./database/monopolyroom.txt', 'utf8', (err, jsonFile) => {
        ...
        fs.writeFile('./database/monopolyroom.txt', JSON.stringify(arr[0]), (err) => {
            if (err) throw err;
...
function updateTextChat(fs, io, data) {
    // console.log("-------------------", data);
    fs.readFile(`./database/start/${data[1]}.txt`, 'utf8', (err, jsonFile) => {
        if (err) {
            return
...
function SendListOfQuestion(fs, io, data) {
    data[2].forEach(e => {
        io.to(e).emit("updateListOfQuestion", data[1])
    })
}

function AddScore(fs, data) {
    fs.readFile(`./database/start/${data[1]}.txt`, 'utf8', (err, jsonFile) => {
        if (err) {
            return
        }
...
function allListen(fs, io, data) {
    ...
    } else if (data[0] === "updateTextChat") {
        updateTextChat(fs, io, data);
    } else if (data[0] === "SendListOfQuestion") {
        SendListOfQuestion(fs, io, data);
    } else if (data[0] === "AddScore") {
        AddScore(fs, data);
